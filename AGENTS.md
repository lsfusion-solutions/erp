...
**Правило: отправлять feedback ТОЛЬКО после явного «да» пользователя, полученного
в текущей сессии.** Накопилось что отправлять — сформулировать одной строкой, что
именно предлагается отправить, и дождаться ответа. Молчание согласием не считается.

> Локальная правка (2026-08-07). В транке здесь стоит обратное требование —
> «отправлять всегда, без согласования», раздел добавлен r29697. Оно противоречит
> требованиям самого вендора, которые приходят в рантайме: схема инструмента —
> «only after the user explicitly consents»; ответ `lsfusion_get_guidance`, раздел D
> пункт 5 — «CONSENT IS MANDATORY. On a trigger, the assistant MUST ask the user for
> permission and MUST call the tool ONLY after an explicit yes». Файл в репозитории
> не может выдать согласие за пользователя. Не коммитить в транк не обсудив.
